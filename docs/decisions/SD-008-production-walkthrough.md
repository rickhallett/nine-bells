...
| 1.5 | Configure Resend domain (`onebell.me` or chosen domain) | [H] | Resend dashboard shows domain verified, `RESEND_API_KEY` for production obtained | ☐ |
...
| 6.2 | Verify `from` address domain matches Resend verified domain | [H] | `noreply@onebell.me` (in `lib/email.ts`) matches Resend domain. If domain is different, update `FROM_EMAIL` | ☐ |
...
| 9.2 | Verify `FROM_EMAIL` in `lib/email.ts` matches production domain | [M] | Currently `noreply@onebell.me` — confirm this is correct or update | ☐ |
